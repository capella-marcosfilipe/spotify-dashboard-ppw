...
// Métodos de Top 5 acústicas ========================

async function fetchSongsByAcousticness() {
  try {
    const url = `https://parseapi.back4app.com/parse/classes/songs?order=-acousticness&limit=5`;

    const songs = await fetchSongs(url);

    renderAcousticnessSongs("acousticness-songs-list", songs);
  } catch (error) {
    handleError(error);
  }
}

function renderAcousticnessSongs(elementId, songs) {
  /**
   * Formata as músicas encontradas em HTML
   */
  const songsList = document.getElementById(elementId);
  ...
  songs.forEach((song) => {
    ...
    listItem.innerHTML = `
      ...
      <span class="fs-6 fw-light">Nível acústico:</span> <span class="fs-6 fw-semibold">${
        song.acousticness
      }</span><br>
      <span class="fs-6 fw-light">Streams:</span> <span class="fs-6 fw-semibold">${song.streams.toLocaleString(
        "pt-br"
